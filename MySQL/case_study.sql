create
database if not exists furama_dn;
use
furama_dn;
create table vi_tri
(
...
(
    ...
    id_trinh_do   int,
    id_bo_phan    int,
    foreign key (id_vi_tri) references vi_tri (ma_vi_tri),
    foreign key (id_trinh_do) references trinh_do (ma_trinh_do),
    foreign key (id_bo_phan) references bo_phan (ma_bo_phan)
);
...
(
    ...
    id_loai_khach int,
    foreign key (id_loai_khach) references loai_khach (ma_loai_khach),
    ho_ten        varchar(45),
    ngay_sinh     date,
    gioi_tinh     bit(1),
    so_cmnd       varchar(45),
...
(
    ...
    id_kieu_thue            int,
    id_loai_dich_vu         int,
    foreign key (id_kieu_thue) references kieu_thue (ma_kieu_thue),
    foreign key (id_loai_dich_vu) references loai_dich_vu (ma_loai_dich_vu),
...
(
    ...
    id_khach_hang     int,
    id_dich_vu        int,
    foreign key (id_nhan_vien) references nhan_vien (ma_nhan_vien),
    foreign key (id_khach_hang) references khach_hang (ma_khach_hang),
    foreign key (id_dich_vu) references dich_vu (ma_dich_vu)
);
...
(
    ...
    foreign key (id_hop_dong) references hop_dong (ma_hop_dong),
    foreign key (id_dich_vu_di_kem) references dich_vu_di_kem (ma_dich_vu_di_kem),
    so_luong             int
);
...
values (1, 'Villa Beach Front', 25000, 10000000, 10, 3, 1, 'vip', 'Có hồ bơi', 500, 4, null),
       ...
       (3, 'Room Twin 01', 5000, 1000000, 2, 4, 3, 'normal', 'Có tivi', null, null, null),
       ...
       (6, 'Room Twin 02', 3000, 900000, 2, 4, 3, 'normal', 'Có tivi', null, null, null);
